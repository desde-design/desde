 ...
 *
 * Because the viewBox is tight, the round letters' overshoot below the
 * baseline sits exactly on the SVG's bottom edge. An inline `<svg>` clips to
 * its viewport by default, and Chromium snaps that clip to whole device pixels
 * while the box lands at a fractional height (13.8125px at 19px). The bottom
 * sub-pixel of every bowl fell outside the snapped clip, so D, e, s and d all
 * rendered flat-bottomed in the desktop launcher (2026-09-08; reproduced at 4x
 * with the clip on and off). The box is for layout only. Nothing needs to clip
 * the mark, so let the ink spill the sub-pixel it needs. `wordmark.test.tsx`
 * pins this.
 */
const DESDE_PATH =
  ...
  return (
    ...
    >
      <svg
        ...
        aria-hidden="true"
        focusable="false"
        style={{ height: `${INK_EM}em`, width: "auto", display: "block", overflow: "visible" }}
      >
