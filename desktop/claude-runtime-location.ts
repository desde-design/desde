 ...
 *
 * **This file is deleted together with the desktop claude-runtime
 * installer** (a later task in the chat-runtime-consolidation series — see
 * `tasks/chat-runtime-consolidation.md`). It is a desktop-local COPY of what
 * used to be the shared `src/editor/llm-providers/claude-runtime-location.ts`
 * module. That shared module was deleted in task 24 of that series: the
 * Claude Agent SDK lane became a dev-only sidecar that spawns whatever
 * `claude` binary is on PATH (`src/editor/agent-chat-sidecar/
 * resolve-claude-on-path.ts`), so root `src/` no longer has any reason to
 * know about a desktop-managed, version-keyed runtime directory. Desktop's
 * own installer (`desktop/claude-runtime-installer.ts`) still does, until it
 * is removed too — hence this copy, scoped to `desktop/` only.
 *
 ...
 * Computed by hand (not via `app.getPath`) because this module used to be
 * imported by the shared `src/` tree, which must also run OUTSIDE Electron
 * (the terminal CLI, plain `vitest`) — kept that way even as a desktop-local
 * copy, so the two copies stay byte-comparable until this one is deleted.
 *
