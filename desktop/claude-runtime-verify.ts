 ...
 * used to be the shared `src/editor/llm-providers/claude-runtime-verify.ts`
 ...
 * resolve-claude-on-path.ts`), with no bundled/downloaded runtime and so
 * nothing left to verify on that side. Desktop's own installer
 * (`desktop/claude-runtime-installer.ts`) still downloads and verifies a
 * runtime, until it is removed too — hence this copy, scoped to `desktop/`
 * only.
 *
