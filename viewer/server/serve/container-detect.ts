 ...
 *
 * There is deliberately no second heuristic for HOW the container is
 * networked. Rounds 6, 10 and 18 of the codex review each tried to read
 * `/proc/net/dev` for it (Docker's bridge names; then `lo` + `eth<N>` only as
 * positive evidence of a bridge), and round 31 closed the question: a
 * `--network host` container on a host whose NIC is named `eth0` lists
 * exactly `lo` + `eth0`, the same as a bridged one, so no listing proves
 * bridging. The bind is the operator's explicit `VIEWER_LOOPBACK_BIND`
 * (the image says `all`), and the banner tells every container under `all`
 * what host networking needs, whatever its interfaces are called.
 *
