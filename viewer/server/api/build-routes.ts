...
export function createBuildRoutes(deps: AppDeps): Router {
  ...
  router.get("/deployments/:id/log/stream", async (req, res) => {
    ...
    if (closed) return
    if (!deployment) {
      // The SAME body `requireProjectManageRead` sends for an unreadable
      // project (`readableProjectWithPolicy` in `auth/authorize.ts`), not a
      // distinct "Deployment not found" — a caller who cannot read the
      // project this id belongs to must not be able to tell "wrong id" from
      // "right id, no access" apart from the response body. See the paired
      // `denied`/`missing` test below.
  ...
  router.get("/deployments/:id/server-log", async (req, res) => {
    ...
    const project = deployment
      ...
      : null
    if (!deployment) {
      // Same body as the log-stream route above, and for the same reason:
      // it must match whatever `requireProjectManageRead` sends for an
      // unreadable project, so an unknown deployment id and a real one on a
      // project the caller cannot manage are indistinguishable.
      res.status(404).json({ error: "Project not found" })
      return
    }
    if (!project) return
